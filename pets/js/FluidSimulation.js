...
export class FluidSimulation {
 ...
 _setupRenderer(canvas) {
  ...
  // ✅ If canvas has no size yet, use its parent's size instead
  const rect = canvas.getBoundingClientRect();
  const w = rect.width  || canvas.parentElement.offsetWidth;
  const h = rect.height || canvas.parentElement.offsetHeight;
  ...
  window.addEventListener("resize", () => {
    ...
    const w = rect.width  || this.canvas.parentElement.offsetWidth;
    const h = rect.height || this.canvas.parentElement.offsetHeight;
